...
0.1 の中心は `RunGraph` です。`RunGraph` が run 全体の DAG を持ち、`GraphView` がその部分集合を表します。CLI では `GraphView` を `branch` と呼びます。
...
隔離した探索をしたい場合は branch を作ります。branch は global records をコピーせず、`RunGraph` の record membership だけを持つ `GraphView` です。
...
- `GraphView`: `RunGraph` の部分集合。CLI では branch。
